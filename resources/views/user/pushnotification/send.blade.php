...
@section('content')
  ...
  <div class="row">
    <div class="col-md-12">
      ...
      <div class="card">
        <form class="" action="{{ route('user.pushnotification.push') }}" method="post">
          @csrf
          <div class="card-header">
            <div class="card-title">{{ __('Send Push Notification') }}</div>
            <div class="card-category">
              <span class="badge badge-info">{{ __('Active Subscribers') }}: {{ $subscribers_count }}</span>
            </div>
          </div>
          <div class="card-body">
            <div class="row">
          ...
          <div class="card-footer text-center">
            <button type="submit" class="btn btn-success" {{ $subscribers_count == 0 ? 'disabled' : '' }}>
