          ...
          <div class="row">
            ...
            <div class="col-lg-4">
              @if (!empty($userLanguages))
                <select name="language" id="testimonial_language" class="form-control"
          ...
          <form id="testimonialSectionForm" action="{{ route('user.about_us.testimonials.section_info.update') }}"
            ...
            <div class="form-group">
              <label class="d-flex align-items-center justify-content-between">
                <span>{{ __('Title') }}</span>
                <button type="button" class="btn btn-sm btn-primary ai-field-btn" data-field="testimonial_section_title"
            ...
            <div class="form-group">
              <label class="d-flex align-items-center justify-content-between">
                ...
                <button type="button" class="btn btn-sm btn-primary ai-field-btn"
                  data-field="testimonial_section_subtitle" data-lang="" data-title="{{ __('Subtitle') }}">
                  <i class="fas fa-magic"></i> {{ __('Generate') }}
                </button>
              </label>
...
@section('scripts')
  ...
  <script>
    ...
    function updateTestimonialAiLang() {
      var code = $('#testimonial_language option:selected').data('code') || $('#testimonial_language').val() || '';
      $('.ai-field-btn').attr('data-lang', code);
    }

    $(document).on('change', '#testimonial_language', updateTestimonialAiLang);
    updateTestimonialAiLang();

    var testimonialAiScope = null;
    AiFormGenerator.init({
      ...
      context: {},
      outputs: function() {
        if (!testimonialAiScope || !testimonialAiScope.length) return {};
        return {
          testimonial_section_title: testimonialAiScope.find('[name="testimonial_section_title"]'),
          testimonial_section_subtitle: testimonialAiScope.find('[name="testimonial_section_subtitle"]')
        };
      },
      inputSuffix: '',
      ...
      onOpen: function($btn) {
        testimonialAiScope = $btn.closest('form');
        if (!testimonialAiScope.length) {
          testimonialAiScope = $btn.closest('.modal');
        }
      }
    });
  </script>
