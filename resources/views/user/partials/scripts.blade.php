<script>
  ...
  var to = "{{ __('to') }}";
  var ofText = "{{ __('of') }}";
  var imagePrompt = "{{ __('Please describe your image idea') }}";
  var imageGenerating = "{{ __('Generating') }}";
</script>
<!--   Core JS Files   -->
...
<script>
  (function ($) {
    ...
    function normalizeStack() {
      var base = getMaxZ();
      if (base < 1050) base = 1050;

      var $backdrops = $('.modal-backdrop');

      modalStack.forEach(function (modalEl, i) {
        var $m = $(modalEl);
        if (!$m.length) return;

        if (!$m.parent().is('body')) $m.appendTo('body');

        var zModal = base + (i + 1) * 20;
        $m.css('z-index', zModal);

        var backdropIndex = $backdrops.length - modalStack.length + i;
        if (backdropIndex >= 0) {
          $($backdrops.get(backdropIndex))
            .css('z-index', zModal - 10)
    ...
    $(document).on('hidden.bs.modal', '.modal', function () {
      var idx = modalStack.indexOf(this);
