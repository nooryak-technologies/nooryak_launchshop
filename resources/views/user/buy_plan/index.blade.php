        ...
        <div class="card-pricing2 @if (isset($current_package->id) && $current_package->id === $package->id) card-success @else card-primary @endif">
          ...
          <ul class="pricing-content">
            ...
            @if (is_array($allPfeatures) && in_array('AI Content & Image Generator', $allPfeatures))
              @if ($aiFeatureEnabled)
                <li class="ai-parent">{{ __('AI Content & Image Generator') }}</li>
                <li class="ai-child">{{ __('AI Engine') }} :
                  {{ $package->ai_engine ? strtoupper($package->ai_engine) : __('N/A') }}</li>
                <li class="ai-child">{{ __('AI Token Limit') }} :
                  {{ $aiTokenLimitLabel }}</li>
                <li class="ai-child">{{ __('AI Image Limit') }} :
