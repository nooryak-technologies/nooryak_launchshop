    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
      <div class="modal-content">
        <div class="modal-header">
          ...
          <button type="button" class="btn btn-primary btn-sm ml-2 ai-field-btn" data-field="faq" data-lang=""
            data-title="{{ __('Generate FAQ') }}">
            <i class="fas fa-magic mr-1"></i> {{ __('Generate Q&A') }}
          </button>
        ...
        <div class="modal-body">
          <form id="ajaxForm" class="modal-form create" action="{{ route('user.faq.store') }}" method="POST">
            ...
            <div class="form-group">
              ...
              <select name="user_language_id" id="faq_language" class="form-control">
                <option value="" selected disabled>{{ __('Select a language') }}
                </option>
                @foreach ($userLanguages as $lang)
...
@section('scripts')
  ...
  <script>
    ...
    function updateFaqAiLang() {
      var code = $('#faq_language option:selected').data('code') || '';
      $('#createModal .ai-field-btn').attr('data-lang', code);
    }

    $(document).on('change', '#faq_language', updateFaqAiLang);
    $('#createModal').on('shown.bs.modal', updateFaqAiLang);
    updateFaqAiLang();

    var faqAiScope = null;
    AiFormGenerator.init({
      openBtn: '#createModal .ai-field-btn',
      ...
      context: {},
      outputs: function() {
        if (!faqAiScope || !faqAiScope.length) return {};
        return {
          question: faqAiScope.find('[name="question"]'),
          answer: faqAiScope.find('[name="answer"]')
        };
      },
      inputSuffix: '',
      ...
      onOpen: function($btn) {
        faqAiScope = $btn.closest('form');
        if (!faqAiScope.length) {
          faqAiScope = $('#createModal');
        }
      }
    });
  </script>
