...
<div id="pwa-install-banner" class="pwa-install-banner-bar" style="display:none;">
  ...
  <div style="display:flex;align-items:center;gap:10px;">
    <button type="button" id="pwa-install-btn" onclick="triggerPwaInstall()"
            style="background:#0f172a;color:#fff;border:none;border-radius:6px;padding:8px 18px;font-size:13px;font-weight:600;cursor:pointer;display:inline-flex;align-items:center;gap:6px;outline:none;">
    ...
    <button type="button" onclick="dismissPwaBanner()"
            style="background:transparent;border:none;font-size:22px;color:#94a3b8;cursor:pointer;padding:0 4px;line-height:1;">&times;</button>
  </div>
</div>
...
<script>
  ...
  // Capture the install event BEFORE page fully loads
  window.addEventListener('beforeinstallprompt', function(e) {
    e.preventDefault();
    deferredPwaPrompt = e;
    // Show the sticky banner only when browser confirms installable
  ...
  // If already installed as app, hide banner
  ...
  function triggerPwaInstall() {
    ...
    deferredPwaPrompt.prompt(); // Fires browser native "Install app" dialog
