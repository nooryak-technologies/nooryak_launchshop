    ...
    <div class="row align-items-center justify-content-between">
      <div class="col-lg-4 col-md-5 col-sm-12">
        <div class="content text-start">
          <h2 style="{{ $isUrbanTheme ? 'font-weight: 600; color: #1a1a1a; letter-spacing: -0.3px;' : '' }}">@yield('breadcrumb_title')</h2>
          <nav aria-label="breadcrumb">
            <ol class="breadcrumb justify-content-start mb-0" style="background: transparent; padding: 0;">
              <li class="breadcrumb-item"><a
                  href="{{ route('front.user.detail.view', getParam()) }}" style="{{ $isUrbanTheme ? 'color: #666; text-decoration: none;' : '' }}">{{ $keywords['Home'] ?? __('Home') }}</a></li>
              <li class="breadcrumb-item active" aria-current="page" style="{{ $isUrbanTheme ? 'color: #a87d52; font-weight: 500;' : '' }}">
                @if (
