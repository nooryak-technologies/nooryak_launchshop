!(function ($) {
    ...
    $(document).ready(function () {
        ...
        // Global handler for clicks on links/buttons inside popups to prevent them showing again
        $(document).on('click', '.popup-wrapper a', function (e) {
            let $popup = $(this).closest('.popup-wrapper');
            let popupId = $popup.data('popup_id');
...
function popupAnnouncement($this) {
    ...
    // Check if dismissed permanently via localStorage
    if (localStorage.getItem('popup_dismissed_' + popupId)) {
    ...
    if (closedPopups.indexOf(popupId) == -1) {
        ...
        setTimeout(function () {
            jQuery.magnificPopup.open({
                ...
                type: 'inline',
                fixedContentPos: true, // Prevent background scrolling on mobile
                callbacks: {
                    afterClose: function () {
                        // after the popup is closed, store it in the sessionStorage, mark dismissed in localStorage, & show next popup
                        localStorage.setItem('popup_dismissed_' + popupId, 'true');
                        closedPopups.push(popupId);
                        sessionStorage.setItem('closedPopups', JSON.stringify(closedPopups));
