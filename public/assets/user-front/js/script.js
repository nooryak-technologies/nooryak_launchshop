...
!(function ($) {
    ...
    if ($('.cat-slider-clothing').length > 0) {
        $(".cat-slider-clothing").each(function () {
            var id = $(this).attr("id");
            var sliderId = "#" + id;
            $(sliderId).slick({
                dots: true,
    ...
    if ($('.top-rated-slider-clothing').length > 0) {
        $(".top-rated-slider-clothing").each(function () {
            var id = $(this).attr("id");
            var sliderId = "#" + id;
            $(sliderId).slick({
                dots: true,
    ...
    // On mobile, auto-scroll banner rows to cycle through the second banner image.
    function initMobileBannerAutoScroll() {
        var isMobileView = window.matchMedia('(max-width: 575px)').matches;
        var bannerRows = $('.section-banner .row, .banner-collection .row');

        bannerRows.each(function () {
            var $row = $(this);
            var slideCount = $row.children('[class*="col-"]').length;
    ...
    initMobileBannerAutoScroll();
    $(window).on('resize', initMobileBannerAutoScroll);
