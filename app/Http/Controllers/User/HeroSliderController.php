...
class HeroSliderController extends Controller
{
    ...
    public function updateHeroSecBgImg(Request $request)
    {
        ...
        if (
            empty($data->hero_section_background_image) &&
            !$request->hasFile('hero_section_background_image') &&
            empty($request->ai_generated_hero_section_background_image)
        ) {
        ...
        if ($request->hasFile('hero_section_background_image')) {
            ...
            $destDir = public_path('assets/front/img/hero_slider/');
            if (!empty($image) && file_exists($destDir . DIRECTORY_SEPARATOR . $image)) {
                @unlink($destDir . DIRECTORY_SEPARATOR . $image);
            }
