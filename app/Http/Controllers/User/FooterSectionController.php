...
class FooterSectionController extends Controller
{
    ...
    public function update(Request $request, $langid)
    {
        ...
        $rules = [
            ...
            'ai_generated_footer_logo' => 'nullable|string',
            'ai_generated_footer_background_image' => 'nullable|string',
            'file' => [
        ...
        } elseif (!empty($request->ai_generated_footer_logo)) {
            $dir = public_path('assets/front/img/footer/');
            if (!empty($bs->footer_logo) && file_exists($dir . DIRECTORY_SEPARATOR . $bs->footer_logo)) {
                @unlink($dir . DIRECTORY_SEPARATOR . $bs->footer_logo);
            }
            $footer_logo = moveAiStorageImageToPublicAssets($request->ai_generated_footer_logo, $dir);
        } else {
        ...
        } elseif (!empty($request->ai_generated_footer_background_image)) {
            $dir = public_path('assets/front/img/footer/');
            @mkdir($dir, 0775, true);
            if (!empty($bs->background_image) && file_exists($dir . DIRECTORY_SEPARATOR . $bs->background_image)) {
                @unlink($dir . DIRECTORY_SEPARATOR . $bs->background_image);
            }
            $background_image = moveAiStorageImageToPublicAssets(
                $request->ai_generated_footer_background_image,
                $dir
            );
        } else {
