...
class AboutUsController extends Controller
{
    ...
    public function updaetAbout(Request $request)
    {
        ...
        if ($request->hasFile('image')) {
            $in['image'] = Uploader::update_picture($destDir, $request->file('image'), @$aboutUs->image);
        } elseif (!empty($request->ai_generated_image)) {
            if (!empty(@$aboutUs->image) && file_exists($destDir . DIRECTORY_SEPARATOR . $aboutUs->image)) {
                @unlink($destDir . DIRECTORY_SEPARATOR . $aboutUs->image);
            }
