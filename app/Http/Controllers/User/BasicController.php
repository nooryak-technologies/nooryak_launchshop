...
class BasicController extends Controller
{
    ...
    public function updatebreadcrumb(Request $request)
    {
        ...
        $rules = [
            'ai_generated_breadcrumb' => 'nullable|string',
            'breadcrumb' => [
        ...
        } elseif (!empty($request->ai_generated_breadcrumb)) {
            $filename = moveAiStorageImageToPublicAssets($request->ai_generated_breadcrumb, $dir);
        }

        if (!empty($filename)) {
            if (!is_null($bss) && $bss->breadcrumb) {
                @unlink($dir . $bss->breadcrumb);
            }

            if (is_null($bss)) {
                $bss = new BasicExtende();
                $bss->language_id = $userCurrentLang;
                $bss->user_id = $authId;
            }
    ...
    public function updateUserNotFoundPage(Request $request)
    {
        ...
        if (
            !$request->hasFile('page_not_found_image') &&
            empty($request->ai_generated_image) &&
            (is_null($data) || is_null($data->page_not_found_image))
        ) {
        ...
        if ($request->hasFile('page_not_found_image')) {
            ...
            $directory = public_path('assets/user-front/images/');
            if (!empty($imageName) && file_exists($directory . DIRECTORY_SEPARATOR . $imageName)) {
                @unlink($directory . DIRECTORY_SEPARATOR . $imageName);
            }
            $imageName = moveAiStorageImageToPublicAssets($request->ai_generated_image, $directory);
        }

        // Update BasicExtende
        BasicExtende::updateOrInsert(
            ['user_id' => $user->id, 'language_id' => $language->id],
            $request->except(['_token', 'page_not_found_image', 'ai_generated_image']) + [
