...
class CounterInformationController extends Controller
{
    ...
    public function updateInfo(Request $request)
    {
        ...
        $in = $request->all();
        unset($in['ai_generated_image']);
        $counterInfo = CounterSection::where([['language_id', $lang_id], ['user_id', $user_id]])->first();
        $destDir = public_path('assets/front/img/user/about');
        if ($request->hasFile('image')) {
            $in['image'] = Uploader::update_picture($destDir, $request->file('image'), @$counterInfo->image);
        } elseif (!empty($request->ai_generated_image)) {
            if (!empty(@$counterInfo->image) && file_exists($destDir . DIRECTORY_SEPARATOR . $counterInfo->image)) {
                @unlink($destDir . DIRECTORY_SEPARATOR . $counterInfo->image);
            }
            $in['image'] = moveAiStorageImageToPublicAssets($request->ai_generated_image, $destDir);
        }
