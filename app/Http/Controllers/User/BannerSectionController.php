...
class BannerSectionController extends Controller
{
    ...
    public function storebanner(Request $request)
    {
        ...
        $destDir = public_path('assets/front/img/user/banners');
        if ($request->hasFile('banner_img')) {
            $request['image_name'] = Uploader::upload_picture($destDir, $request->file('banner_img'));
        } elseif (!empty($request->ai_generated_banner_img)) {
            $request['image_name'] = moveAiStorageImageToPublicAssets(
                $request->ai_generated_banner_img,
                $destDir
            );
        }
        banner::create($request->except('banner_img', 'user_id', 'ai_generated_banner_img') + [
